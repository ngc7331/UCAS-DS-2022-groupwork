...
struct Edge {
    ...
    Edge() {}
    Edge(int _id, int _s, int _t, int _sTime, int _tTime, int _cost)
        : id(_id)
        , s(_s)
        , t(_t)
        , sTime(_sTime)
...
struct ArrEdge {
    ...
    ArrEdge() {}
    ArrEdge(int _id, int _tTime)
        : id(_id)
        , tTime(_tTime) {}
};
bool operator<(ArrEdge a, ArrEdge b) {
    return a.tTime != b.tTime ? a.tTime < b.tTime : a.id < b.id;
}
...
struct State {
    ...
    State() {}
    State(ArrEdge _arr, int _cost, int _pre, int _index)
        : arr(_arr)
        , cost(_cost)
        , pre(_pre)
        , index(_index) {}
...
Status DP ::newRoute(int id, ROUTE_TYPE type, int s, int t, int sTime, int duration, int cost) {
    ...
    if (edge[type].find(tmp) != edge[type].end())
        return ERR_VALUE;
...
void work(int S, int T, ROUTE_TYPE routeType, POLICY_TYPE policyType) {
    for (auto i : edge[routeType]) {
        int index = upper_bound(f[i.s].begin(), f[i.s].end(), State(ArrEdge(0, i.sTime), 0, 0, 0)) -
                    f[i.s].begin() - 1;
        if (index < 0 || f[i.s][index].arr.tTime > i.sTime)
            continue;
        ...
        for (auto i : edge[routeType]) {
            if (i.s == S) {
                ...
                if (f[T].empty())
                    continue;
                int tmp = 0;
                while (tmp + 1 < f[T].size() && f[T][tmp + 1].arr.tTime == f[T][0].arr.tTime)
                    ++tmp;
                if (f[T][tmp].arr.tTime - i.sTime < time ||
                    (f[T][tmp].arr.tTime - i.sTime == time && f[T][tmp].cost < cost)) {
