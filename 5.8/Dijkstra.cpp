...
// args:   int city_id
// return: Status code
Status Dijkstra::delCity(int cityID) { // 删除该点的相应边由前端实现
...
// args:   int cost
// return: Status code
Status Dijkstra::newRoute(int routeID, ROUTE_TYPE routeType, int startCityID, int endCityID,
                          int startTime, int duration, int cost) {
    DEBUG(std::cout << "newRoute() called: " << routeID << " " << routeType << " " << startCityID
                    << " " << endCityID << " " << startTime << " " << duration << " " << cost
                    << std::endl;)
...
// args:   ROUTE_TYPE route_type
// return: Status code
Status Dijkstra::delRoute(int routeID, ROUTE_TYPE routeType) { // 删除指定的边
    NodeLink *bp, *p;
    for (auto startCity : Map.city) // 遍历Map中的city
    {
        if (startCity.existence && startCity.first) {
            ...
            while (p) {
                if (p->routeInfo.routeID == routeID &&
                    p->routeInfo.routeType == routeType) { // 删除需要删除的边，并更新全局计数器
                    bp->next = p->next;
                    delete p;
                    Map.edgeNum--;
                    return OK;
...
std::vector<int>
Dijkstra::search(int startCityID, int endCityID, ROUTE_TYPE routeType,
                 POLICY_TYPE decisionKind) { // 采用堆优化的Dijkstra算法，计算三种情况下的值
    DEBUG(std::cout << "DijkstraSearch() called: "
                    << "startCityID:" << startCityID << ", endCityID: " << endCityID
                    << ", routeType: " << (routeType == PLANE ? "PLANE" : "TRAIN")
                    << ", decisionKind: "
                    << ((decisionKind == COST)   ? "COST"
                        : (decisionKind == TIME) ? "TIME"
                                                 : "INTERCHANGE")
                    << std::endl; // Debug
    )

    if (startCityID < 0 || startCityID >= Map.city.size() || endCityID < 0 ||
        endCityID >= Map.city.size()) {
        std::cout << "err: Overflow" << std::endl; // Debug
        return std::vector<int>(0);                // 合法性检查
    }
    if (!Map.city[startCityID].existence || !Map.city[endCityID].existence ||
        !Map.city[startCityID].first) {
    ...
    std::priority_queue<std::pair<int, RouteInfo>, std::vector<std::pair<int, RouteInfo>>,
    ...
    while (!q.empty()) {
        ...
        for (NodeLink* p = Map.city[lCity].first; p; p = p->next) {
            if (p->routeInfo.routeType == routeType) {
                ...
                if (decisionKind == TIME) { // 在时间函数中，返回值是线路的到达时间，所以不需要累加
                    if (searchDirectRoute(lCity, i, comeRoute, decisionKind) < minCost[i]) {
                        minCost[i] = searchDirectRoute(lCity, i, comeRoute, decisionKind);
                        DEBUG(std::cout << "\t\tupdata minCost[" << i << "] = " << minCost[i]
                                        << std::endl;) // Debug
                        q.push(std::make_pair(minCost[i], p->routeInfo));
                    }
                } else if (lCity == startCityID &&
                           decisionKind == INTERCHANGE) { // 特殊处理：在始发站的直达站，换乘均为0
                    int i = p->routeInfo.endCityIndex;
                    minCost[i] = 0;
                    DEBUG(std::cout << "\t\tupdata minCost[" << i << "] = " << minCost[i]
                                    << std::endl;) // Debug
                    q.push(std::make_pair(minCost[i], p->routeInfo));
                } else if (minCost[lCity] + searchDirectRoute(lCity, i, comeRoute, decisionKind) <
                           minCost[i]) { // 在花费和换乘计算中，返回值是增量，所以需要累加
                    minCost[i] =
                        minCost[lCity] + searchDirectRoute(lCity, i, comeRoute, decisionKind);
                    DEBUG(std::cout << "\t\tupdata minCost[" << i << "] = " << minCost[i]
                                    << std::endl;) // Debug
                    q.push(std::make_pair(minCost[i], p->routeInfo));
                }
...
int searchDirectRoute(int startCity, int endCity, RouteInfo comeRoute, POLICY_TYPE decisionKind) {
    ...
    int res = MAXWEIGHT;
    if (startCity < 0 || startCity >= Map.city.size() || endCity < 0 ||
        endCity >= Map.city.size() || !Map.city[startCity].existence ||
        !Map.city[endCity].existence)
    ...
    for (NodeLink* p = Map.city[startCity].first; p; p = p->next) {
        if (p->routeInfo.endCityIndex == endCity && p->routeInfo.routeType == routeType) {
            ...
            case INTERCHANGE: { // 不能计算startCity本事就是起点站的例子，需要外部函数额外计算
